...
const SidebarSection = ({ title, children, variant }: { title: string; children: React.ReactNode; variant?: 'danger' }) => (
  <div style={{
    ...
    <h3 style={{
      ...
      fontWeight: 600,
      color: variant === 'danger' ? 'var(--color-criticality-critical-text)' : 'var(--color-text-muted)',
...
const InfoItem = ({ label, value, mono, highlight }: { label: string; value: string | number; mono?: boolean; highlight?: boolean }) => (
  <div style={{
    display: 'flex',
...
const ParcelaRow = ({ parcela }: { parcela: Parcela }) => {
  ...
  const rowStyle: React.CSSProperties = {
    ...
    borderLeft: isOverdue ? '2px solid var(--color-criticality-critical-border)' : '2px solid transparent',
  };
  ...
  return (
    <tr style={rowStyle}>
      ...
      <td style={{ ...cellStyle, fontFamily: 'var(--font-mono)', textAlign: 'center', color: isOverdue ? 'var(--color-criticality-critical-text)' : 'var(--color-text-muted)' }}>
  ...
  <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', flex: 1 }}>
    <div style={{ textAlign: 'center' }}>
      <div style={{
        ...
        animation: 'spin 1s linear infinite', margin: '0 auto var(--spacing-sm)',
      }} />
...
const ErrorState = ({ message, onBack }: { message: string; onBack: () => void }) => (
  <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', flex: 1 }}>
    <div style={{ textAlign: 'center' }}>
      <p style={{ color: 'var(--color-criticality-critical-text)', marginBottom: 'var(--spacing-md)', fontSize: 'var(--text-sm)' }}>{message}</p>
...
const ContractDetailPage = () => {
  ...
  // Filter only overdue parcelas for the main table
  ...
  const sidebarContentStyle: React.CSSProperties = {
    padding: isSidebarOpen ? 'var(--spacing-md)' : '0',
    ...
    <div style={pageStyle}>
      ...
      <aside style={sidebarStyle}>
        ...
        <div style={sidebarContentStyle}>
          {/* Resumo do Atraso */}
          <SidebarSection title="Resumo do Atraso" variant="danger">
          ...
          {/* Integrador */}
          <SidebarSection title="Integrador">
